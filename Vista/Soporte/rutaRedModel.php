...
switch($opcion){
    case 1:
        $consulta  = "INSERT INTO articulos (folio_art, nombre_art, modelo_art, marca_art, descripcion_art, numPoliza_art,proveedor_art,categoria,estado_art) VALUES('$folio', '$nombre', '$modelo', '$marca', '$descripcion', '$num_poliza',  '$proveedor','$categoria', '1') ";			
...
function getParcheo(){
    $conect = mysqli_connect("localhost","root","","sistema_urse");
     $conect->set_charset("utf8");
     $qry_vlan="SELECT folio_art FROM articulos WHERE estado_art=1 and categoria=4 and nombre_art LIKE '%panel%';";
	 if ($resultado = mysqli_query($conect, $qry_vlan)) {
	 /* obtener array asociativo */
	 while ($row = mysqli_fetch_assoc($resultado)) {
